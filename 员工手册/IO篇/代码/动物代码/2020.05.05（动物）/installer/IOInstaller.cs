...
namespace 代码.installer
{
    class IOInstaller : BindingInstaller
    {
        ...
        {
            Container.Bind<可以从动物园读取动物的>().To<_使用json读动物园>().AsTransient();
            ...
            Container.Bind<可以唱的>().To<_动物园>().AsTransient();
            Container.Bind<可以读取的>().To<_存取>().AsTransient();
            Container.Bind<可以显示的>().To<_在控制台缓存>().AsTransient();
            Container.Bind<可以存的>().To<_存取>().AsTransient().WhenInjectedInto<_在控制台缓存>();
