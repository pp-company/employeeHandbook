...
git config --global alias.cm "!f() { HEAD=$(git rev-parse --abbrev-ref HEAD); if [ $HEAD == master ] ; then echo 严禁在master上cm，如果是解决冲突，建议手动add、commit; else git add -A;git commit -am $1; fi; unset HEAD;}; f"
...
git config --global alias.start "!git pl;flag="$?"; f() { if [ $flag == 0 ] ;then git checkout master; git nb $1;else echo 更新失败，请检查; fi; unset flag;}; f"
...
git config --global alias.end "!git pl;flag="$?"; f() { if [ $flag == 0 ] ;then git checkout master; git m ;else echo 更新失败，请检查; fi; unset flag;}; f"
